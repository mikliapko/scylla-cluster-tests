...
class ManagerBackupTests(ManagerRestoreTests):
    ...
    def test_worm_backup(self):
        ...
        self.create_worm_bucket(region=self.params.gce_datacenters[0], bucket_name=worm_bucket_name)

        try:
            self.log.info("Wait for backup location accessibility after bucket creation")

            def check_location(node):
                result = node.remoter.run(
                    f"scylla-manager-agent check-location -L {location} --debug",
                    ignore_status=True,
                )
                return result.ok and not result.stderr.strip()

            for node in self.db_cluster.nodes:
                wait_for(
                    func=lambda n=node: check_location(n),
                    step=10,
                    timeout=180,
                    text=f"Waiting for {node.name} to access gs://{worm_bucket_name}",
                )
            ...
            snapshot_files = self.get_all_snapshot_files(cluster_id=mgr_cluster.id, bucket_location=worm_bucket_name)
            ...
            for file_path in snapshot_files:
                blob = storage_client.bucket(worm_bucket_name).blob(file_path)
                try:
                    blob.delete()
            ...
            self.destroy_worm_bucket(bucket_name=worm_bucket_name)
