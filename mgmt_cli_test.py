...
class MgmtCliTest(BackupFunctionsMixIn, ClusterTester):
    ...
    def test_repair_multiple_keyspace_types(self, host_parallelism: int = None):  # pylint: disable=invalid-name
        manager_tool = mgmt.get_scylla_manager_tool(manager_node=self.monitors.nodes[0])
        test_name = 'test_repair_multiple_keyspace_types'
        if host_parallelism is not None:
            test_name = 'test_repair_multiple_keyspace_types with single-host-parallelism={}'.format(host_parallelism)
            if not manager_tool.feature('repair', 'single-host-parallelism'):
                self.log.info('skipping {} - `single-host-parallelism` is not supported'.format(test_name))
                return
